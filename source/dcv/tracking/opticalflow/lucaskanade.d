...
class LucasKanadeFlow : SparseOpticalFlow {
     ...
     * 
     * Consider tracking points detected by using Harris, or Shi-Tomasi 
    ...
    in {
        ...
        auto flow = uninitializedArray!(float[2][])(pointCount);
        flow[] = [0.0f, 0.0f];
			...
			foreach(iteration; 0..iterationCount) {
				foreach(i; rb..re) {
					foreach(j; cb..ce) {
						...
						const float ft = cast(float)(linear(next, cast(float)i + f[1], cast(float)j + f[0]) - current[i, j]);
